...
oneTimeSetUp()
{
  ...
  testDir="${SHUNIT_TMPDIR}/testRun_MouseSample_mm10ERCC_refSeq"
  ...
  # take longer.
  sampleName="mouse1.mm10ERCC.refSeq"
  copyResults=0
  if [ ${copyResults} -eq 1 ]; then
    ...
    echo "${script} -s ${sampleName} \
            --fastq1 ${testDataDir}/QCPipeline/mouse1_1.fastq.gz \
            --fastq2 ${testDataDir}/QCPipeline/mouse1_2.fastq.gz \
            --sample-dir ${testDir} --outdir ${outDir} \
            --refmodel mm10ERCC.refSeq \
            > ${stdoutF} 2> ${stderrF}"
    ${script} -s ${sampleName} \
            --fastq1 ${testDataDir}/QCPipeline/mouse1_1.fastq.gz \
            --fastq2 ${testDataDir}/QCPipeline/mouse1_2.fastq.gz \
